...
export class UserService {
  ...
  async updatePassword(ctx: RequestContextDto, id: string, updatePasswordDto: UpdatePasswordDto)
    : Promise<UserEntity> {
  ...
  async resetPassword(ctx: RequestContextDto, id: string, password: string)
    : Promise<UserDto> {
  ...
  initiateUser(ctx: RequestContextDto): Promise<UserEntity[]> {
    this.logger.log(`${this.getUsers.name}Service Called`);
    const users = JSON.parse(fs.readFileSync(`${__dirname}/../../../../../mock-data/users.json`, 'utf-8'));
